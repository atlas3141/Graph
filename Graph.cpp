...
void Graph::add(char a, char b, int newWeight){ //adds an edge to the graph between two existing nodes
  Node* node1 = nodeLookup(a);
  Node* node2 = nodeLookup(b); //look up the nodes by name

  if ( node1 && node2 ){ //add them
...
void Graph::remove(char a){
  ...
  if(!removeNode) return; //lookup the node andmake sure it exists
